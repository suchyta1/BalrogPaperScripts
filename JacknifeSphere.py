...
def AltJackknifeOnSphere(jarrs, jras, jdecs, jfunc, jargs=[], jkwargs={}, jtype='generate', jfile=None, njack=24, generateonly=False, gindex=0, varonly=False, save=None, rtype='n-1', itsave=False):
    jarrs, jras, jdec = ArrFunc(EnforceArray2D, jarrs, jras, jdecs)
    km, jfile = GenerateRegions(jarrs, jras, jdecs, jfile, njack, gindex, jtype)
    if generateonly:
        return jfile

    ind = AssignIndex(jarrs, jras, jdecs, km)
    save, itsave = SetupSave(save, itsave)
    full_j, full_other = FullResults(jfunc, jargs, jkwargs, jarrs) 
    it_j, it_other, frac = Iterate(jfunc, jargs, jkwargs, jarrs, ind, njack, full_j, full_other, rtype, itsave)
    cov_j = GetCov(full_j, it_j, njack, varonly, rtype)

    if itsave is not None:
        ItSave(itsave, [it_j, it_other])

    if save is not None:
        FullSave(save, [full_j, cov_j, full_other] )
